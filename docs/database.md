...
| 1.11 | `transactions`             | Giao dịch thanh toán tổng hợp |
...
| `image_url`  | `text`        | NOT NULL             | URL ảnh (Cloud Storage) |
...
| `plan`       | `enum('monthly','yearly')`     | NOT NULL         | Loại gói                 |
...
| `type`             | `enum('user_subscription','merchant_subscription')` | NOT NULL      | Mục đích thanh toán            |
| `payment_method`   | `enum('vnpay','momo','cash')`                    | NOT NULL         | Phương thức thanh toán         |
| `payment_ref_id`   | `uuid`                                           | NULLABLE         | FK → `payment_vnpay.id` hoặc `payment_momo.id` |
...
|--------------------|---------------|------------|--------------------------------------|
| `id`               | `uuid`        | PK         | ID record                            |
| `transaction_id`   | `uuid`        | FK → `transactions.id` | Giao dịch nội bộ        |
...
| `is_active`  | `boolean`      | DEFAULT `true`   | Trạng thái mã QR          |
| `created_at` | `timestamptz`  | DEFAULT NOW()    | Ngày tạo                  |
